...
> Drei durchgängig ausgearbeitete Projektvorschläge, die den KI-Entwicklungszyklus aus [Entwicklung.md](Entwicklung.md) (Phasen 0–8) und den Claude-Code-Workflow aus [Claude_Code.md](Claude_Code.md) (Setup, Skills, Subagenten, Hooks, Release) an drei konkreten Rust-Projekten mit steigender Komplexität durchspielen. Jedes Projekt nennt für jede Phase eine **konkrete Aufgabe**, einen **Beispiel-Prompt** für Claude Code und die dazugehörigen **Befehle/Werkzeuge**.

---

## Vereinheitlichtes Phasenschema

Um die drei Projekte vergleichbar zu halten, werden die neun Phasen aus [Entwicklung.md](Entwicklung.md) als gemeinsames Gerüst verwendet; die Claude-Code-spezifischen Elemente (Skills, Subagenten, Hooks, Plugins) werden an der jeweils passenden Stelle eingehängt.
...
```mermaid
flowchart LR
    A["1. Einsteiger:\nMarkdown-CLI-Toolkit"] --> B["2. Fortgeschritten:\nAsync REST-API"]
    B --> C["3. Profi:\nVerteilter KV-Store\nmit FFI &amp; WASM"]
```
...
> 💡 **Stretch Goal:** Die Statistik-Logik als separate Library-Crate herauslösen und zusätzlich für `wasm32-unknown-unknown` kompilieren, um sie in einer Browser-Vorschau wiederzuverwenden — Vorgeschmack auf Projekt 3.
...
| 8 | `cargo audit`/`cargo deny` in CI verankern, MSRV pinnen, Security-Audit-Skill aus [Entwicklung.md](Entwicklung.md#8-phase-security-audits-supply-chain--governance) als Claude-Skill nachbilden | „Lege den Skill `.claude/skills/rust_security_audit/SKILL.md` an und binde ihn als Pflichtschritt vor jedem Release ein.“ | `cargo audit`, `cargo deny`, `cargo msrv` |
...
> ⚠️ **Fallstrick:** Den Hintergrund-Worker mit `Arc<Mutex<...>>` „reparieren“, statt das Nebenläufigkeitsmodell (z. B. auf `tokio::sync::mpsc`-Channels) zu überdenken — siehe [Entwicklung.md, Phase 4](Entwicklung.md#4-phase-borrow-checker--concurrency-debugging).
...
> ⚠️ **Fallstrick:** `unsafe` an der FFI-Grenze „weil es nun mal FFI ist“ ungeprüft akzeptieren — jeder `unsafe`-Block braucht eine explizite Sicherheitsbegründung im Kommentar und einen Miri-Test, siehe [Entwicklung.md, Phase 5](Entwicklung.md#5-phase-refactoring-idiomatischer-code--performance-audit).

---

## Wie weiter?

Die drei Projekte bauen bewusst aufeinander auf: `mdstat` etabliert die Grundroutine (Phase 0–2 im Griff haben), `taskflow` erweitert um Concurrency/DB/Observability, `nodemesh` verlangt die volle Bandbreite aus Systems Programming und fortgeschrittener Claude-Code-Orchestrierung (parallele Subagenten, Plugins). Wer alle drei durchläuft, hat jede Phase aus [Entwicklung.md](Entwicklung.md) und jedes Werkzeug aus [Claude_Code.md](Claude_Code.md) mindestens einmal praktisch angewendet.